...
module.exports = {
  async populateQuestion(excel_path, section, Question, res){
    try{
      const rows = await xlsxFile(excel_path);
      let question = [];
      ...
      success_response(res, response, "Create Successful");
    } catch (err) {
      console.error(err);
  ...
  async populateKreapelinQuestion(section, Question, res){
    ...
    const kreapelin_answers = kreapelin_questions.map(column => {
      const answer = column.slice(0, -1).map((val, index) => (val + column[index + 1]) % 10);
      return answer;
    });

    const questions = kreapelin_questions.map((column, index) => ({
      instruction: column.toString(),
      section_id: section,
      option_num: 1,
      option_a: "-",
      option_b: "-",
      ...
      option_d: "-",
      option_e: "-",
      answer: kreapelin_answers[index].toString(),
      option_type: 1,
      instruction_type: 1,
    }));
